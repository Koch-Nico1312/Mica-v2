...
def advanced_agent(
    ...
    def log(message: str) -> None:
        line = f"[AdvancedAgent] {message}"
        try:
            print(line)
        except UnicodeEncodeError:
            # Legacy console codepages (cp1252 and friends) cannot represent the
            # symbols the sub-agents log (✓/✗, box drawing).  Logging an
            # optional character must never take a task down with it.
    ...
    try:
        # Initialize the integration inside the try block: an invalid
        # project_path (for example one outside the code agent's sandbox) has to
        # come back as an error string instead of escaping into the caller.
        integration = AgentIntegration(
            logger=log,
            project_root=p.get("project_path"),
            plugins_dir=Path(__file__).parent.parent / "plugins",
            core_tool_names={"advanced_agent"},  # This action's name
            allowed_roots=p.get("allowed_roots"),
        )
...
"""
            ...
            if not health:
                # An empty report reads like a failure; say what is actually
                # there instead (the loader discovers plugins on construction).
                try:
                    discovered = len(integration.list_plugins())
                except Exception:
                    discovered = 0
                output += (
                    "No plugin process is running "
                    f"({discovered} plugin(s) discovered in the plugins directory).\n"
                )
