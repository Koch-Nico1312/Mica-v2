...
def default_allowed_roots() -> list[Path]:
    """Directories the code agent is allowed to work in.

    Covers the usual developer locations *and* the Mica checkout itself, so the
    agent works on the user's own projects out of the box.  A previous version
    only listed ~/Desktop/JarvisProjects, which rejected every real project -
    including Mica itself.  MICA_CODE_AGENT_ROOTS (os.pathsep separated)
    replaces the defaults for sandboxed or server deployments.
    """
    configured = os.environ.get("MICA_CODE_AGENT_ROOTS", "").strip()
    if configured:
        return [Path(p.strip()).expanduser() for p in configured.split(os.pathsep) if p.strip()]

    home = Path.home()
    return [
        Path(__file__).resolve().parents[2],   # the Mica repository root
        home / "Desktop",
...
# Directory names that never hold project sources.  The previous filter only
# listed ".venv" as an exact name, so ".venv-local", ".venv-kokoro" and
# ".uv-cache" were walked in full and reported as project files.
_EXCLUDED_DIRS = {
    ".git", ".hg", ".svn", "__pycache__", ".pytest_cache", ".mypy_cache",
    ".ruff_cache", ".uv-cache", ".tox", ".nox", "node_modules", "site-packages",
    "dist", "build", "venv", "env", ".venv",
}
_EXCLUDED_DIR_PREFIXES = (".venv", "venv-", "env-", "brag-output-")


def _is_excluded_dir(name: str) -> bool:
    """True for directories that are tooling, caches or vendored code."""
    return name in _EXCLUDED_DIRS or name.startswith(_EXCLUDED_DIR_PREFIXES)


# Words that describe an intent, not a file.  Substring matching used to keep
# them: the token "py" (from "audio_devices.py") matched every .py file, so the
# "top 5 candidates" were simply the first five files in the listing.
_TASK_STOPWORDS = frozenset({
    "the", "and", "for", "with", "from", "that", "this", "into", "when",
    "then", "fix", "fixes", "fixing", "add", "adds", "adding", "change",
    "changes", "update", "updates", "make", "makes", "refactor", "code",
    "file", "files", "line", "lines", "please", "should", "would",
})
_PATH_TOKEN_RE = re.compile(r"[\w./\\-]*\.\w{1,6}\b")


def _requirement_name(line: str) -> str:
    """Reduce a requirements.txt line to its bare package name.

    Environment markers and extras are part of the line, not the name: the old
    parser reported "comtypes; sys_platform" as a dependency.
    """
    ...
    return line.split("[", 1)[0].strip()      # drop extras


def _tokens(text: str, min_length: int = 3, drop: frozenset[str] = frozenset()) -> set[str]:
    """Split text into lower-case identifier tokens."""
    return {
        token
        for token in re.findall(r"[A-Za-z0-9]+", text.lower())
        if len(token) >= min_length and token not in drop
    }
...
class CodeAgent:
    ...
    def __init__(
        ...
        self.allowed_roots = [Path(p).expanduser().resolve() for p in (allowed_roots or default_allowed_roots())]
    ...
    def _walk_files(self) -> Iterator[Path]:
        """
        Yield every file below the project root, skipping tooling directories.

        Uses os.walk with pruned directories instead of Path.rglob: the project
        contains .venv-* and .uv-cache trees with tens of thousands of files that
        rglob descends into before they are filtered out again.
        """
    ...
    def _find_source_files(self, language: str) -> list[Path]:
        ...
        return sorted(f for f in self._walk_files() if f.suffix in extensions)
            ...
            for dep_file in ["requirements.txt", "pyproject.toml", "setup.py"]:
                ...
                if path.exists():
                    if dep_file == "requirements.txt":
                        deps = path.read_text(encoding="utf-8").splitlines()
                        dependencies.extend(
                            name
                            for name in (_requirement_name(d) for d in deps)
                            if name
                        )
...
Tier:"""
    ...
    def _identify_target_files(self, task_description: str) -> list[Path]:
        ...
        known = set(self.context.files)

        # 1. Paths named in the task win outright: "core/audio_devices.py" is an
        #    instruction, and guessing around it only risks editing other files.
        mentioned: list[Path] = []
        for raw in _PATH_TOKEN_RE.findall(task_description):
            candidate = (self.project_root / raw.replace("\\", "/")).resolve()
            if candidate in known and candidate not in mentioned:
                mentioned.append(candidate)
        if mentioned:
            return mentioned[:5]

        # 2. Otherwise rank files by shared name tokens - whole tokens, never
        #    substrings, so "py" cannot match every python file.
        keywords = _tokens(task_description, min_length=4, drop=_TASK_STOPWORDS)
        scored: list[tuple[int, Path]] = []
        for file_path in self.context.files:
            score = len(keywords & _tokens(file_path.stem))
            if score:
        ...
        # 3. Fall back to entry points.
        if self.context.entry_points:
            return [self.project_root / entry for entry in self.context.entry_points]
...
Modified code:"""
            ...
            # A model that answers with nothing (or echoes the file unchanged)
            # must never truncate or rewrite the file.
            if not new_content or not new_content.strip():
                self.logger(f"[CodeAgent] Empty modification for {file_path}; leaving it unchanged")
                continue
    ...
    def _run_tests(self) -> str:
        ...
        language = self.context.language
        test_commands = {
            # sys.executable, not "python": the app usually runs inside
            # .venv-local, where plain "python" on PATH may be a different
            # interpreter without pytest.
            "python": [sys.executable or "python", "-m", "pytest", "-v"],
            "javascript": ["npm", "test"],
            "typescript": ["npm", "test"],
            "go": ["go", "test", "./..."],
            "rust": ["cargo", "test"],
