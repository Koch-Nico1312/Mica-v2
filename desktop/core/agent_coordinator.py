...
Report:"""
    ...
    async def _code_agent(self, task: AgentTask) -> str:
        ...
        if project_path and Path(project_path).resolve() != self.code_agent.project_root:
            # Reuse the existing sandbox: a task must not be able to widen the
            # code agent's allowed roots by simply passing a project_path.
            self.code_agent = CodeAgent(
                project_path,
                allowed_roots=self.code_agent.allowed_roots,
                logger=self.logger,
            )
