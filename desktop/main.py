...
_PROJECT_DIR = _Path(__file__).resolve().parents[1]
_DESKTOP_DIR = _Path(__file__).resolve().parent
for _path in (str(_PROJECT_DIR), str(_DESKTOP_DIR)):
...
class JarvisLive:
    ...
    async def _listen_audio(self):
        ...
        # No recording device must not tear down the whole live session and
        # trigger the reconnect loop. Keep text, tools and playback available;
        # applying a device change already rebuilds the surrounding session.
        if not audio_devices.list_devices("input"):
            message = "Kein Mikrofon verfügbar – Textmodus bleibt aktiv."
