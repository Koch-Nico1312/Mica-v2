...
`config/credential-names.example.json` to `config/credential-names.json`, store
the named values with `core.secure_store`, and launch Compose through
`python -m mica_core.windows_launcher`. This reads Windows Credential Manager
...
docker run --rm -v "$PWD:/workspace:ro" -w /workspace/mica_core mica-core-acceptance python -m unittest tests.test_docker_learning_acceptance -v
```
